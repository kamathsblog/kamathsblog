...
💡 ***Tip:*** *If you are building your own in Europe, consider buying the* [*LeKiwi kit*](https://www.seeedstudio.com/Lekiwi-Kit-p-6501.html?srsltid=AfmBOoor6yoA4jyBRwBoLG-n1eJDYj5YFyvUSzMUODRoAKmlwFBxVD0V) *even if you have a 3D printer. Sourcing the parts individually (especially the wheels) will cost you more than buying the kit directly from a company like* [*SeeedStudio*](https://www.seeedstudio.com/)*.*
...
⭐️ [***STS Series***](https://www.feetechrc.com/sts_ttl_series%20servo.html) *- This series uses TTL communication. They're essentially the same hardware as SMS motors, but with an (updated) TTL-based protocol that offers better features and reliability.*
...
💡 *One example is the* [*STS3032 motor*](https://www.feetechrc.com/6v-45kg-magnetic-code-360-degree-serial-bus-steering-gear.html)*.* [*Homemade Garbage*](https://x.com/H0meMadeGarbage) *(a maker who builds some excellent self-balancing robots) has written more about it, and the* [*SCServo Arduino library*](https://docs.arduino.cc/libraries/scservo/) *in his* [*blog post*](https://homemadegarbage.com/sts3032)*.*
...
⭐️ [***SMS Series***](https://www.feetechrc.com/sms_rs485_series%20servo.html) *- Uses RS485 communication. This is the older protocol designed for industrial environments with longer cable runs and better noise immunity.*

⭐️ [***SCS Series***](https://www.feetechrc.com/scs_ttl_Servo.html) *- Also uses TTL communication, similar to STS but with different command sets and features.*

⭐️ [*HLS Series*](https://www.feetechrc.com/hl%E6%81%92%E5%8A%9B%E7%B3%BB%E5%88%97%E8%88%B5%E6%9C%BA.html) *- Another motor series similar to the STS series using TTL communication, but includes a Torque control mode instead of an open-loop PWM mode.*
...
⚠️ ***Software compatibility:*** *I’ve tested the SCServo\_Linux SDK with STS3215 motors only. But there’s no reason it shouldn’t work with other STS motor variants or motors from the other three protocols. Feetech makes a* [*few other series of motors*](https://www.feetechrc.com/%E4%B8%B2%E5%8F%A3%E7%B3%BB%E5%88%97.html)*, which I’ve not investigated and are most likely not compatible with the SCServo\_Linux package.*
...
0️⃣ ***Mode 0: Position Mode*** *- Standard servo behavior. Command a target position (0-4095 steps for roughly 360 degrees), and the servo moves there. You can also control speed and acceleration during the movement. This is the mode used by the SO arms.*

%[https://www.youtube.com/watch?v=Ej6UaO4XC0M] 

1️⃣ ***Mode 1: Velocity Mode*** *- The servo acts like a continuous rotation motor with closed-loop feedback. You set a target speed, and it maintains that velocity. This is what I am using in the LeKiwi robot.*

%[https://www.youtube.com/watch?v=btUzb09SpA4] 

2️⃣ ***Mode 2: PWM Mode*** *- Direct open-loop torque control. You specify a PWM duty cycle, and the servo applies that much power. This is more responsive but less precise than velocity mode. I haven't used this mode much - I like having the loop closed for precise control.*

%[https://www.youtube.com/watch?v=SaqVMd_az9U] 

3️⃣ ***Mode 3: Step Mode*** *- This mode would theoretically emulate a stepper motor, useful for applications that need step-based motion control. While the hardware supports this mode, the SCServo\_Linux SDK doesn't have functions for it, so I haven't tested it with the STS3215. I also haven't needed to use this mode before, so I did not dig deeper.*
...
🚩 ***Documentation*** *- The original repository had some Chinese comments, but no real documentation. In 2023, I had translated comments on files I had experimented with, but the rest were still in Chinese. The documentation I had added was just bullet points of my personal notes, not very clear.*

🚩 ***Code cleanup*** *- To start with, the original repository was renamed to FTServo\_Linux, and a new protocol, HLSCL, was added. There was a lot of duplicate code; several blocks of code were duplicated in tens of places throughout the codebase. For example, the same direction bit encoding/decoding logic was duplicated in nearly 30 places across different files. Also, the error handling was inconsistent.*
...
⚠️ ***Note:*** *The AI-generated translations and the doxygen-style comments were reviewed in detail and are correct. The docs folder with the four markdown files was created for fun, to see the limitations of AI, and has not been reviewed. For now, these documents have been moved to a separate branch* [`with_ai_docs`](https://github.com/adityakamath/SCServo_Linux/tree/with_ai_docs/docs) *and carry an “AI-generated” disclaimer.*
...
⚙️ [`ServoUtils.h`](https://github.com/adityakamath/SCServo_Linux/blob/main/ServoUtils.h)*: Provides parametrized utility functions to replace nearly 300 lines of duplicated code in 30+ instances.*

⚙️ [`ServoErrors.h`](https://github.com/adityakamath/SCServo_Linux/blob/main/ServoErrors.h)*: Provides structured error codes and result types for consistent error handling throughout the codebase.*

⚙️ [`SyncWriteBuffer.h`](https://github.com/adityakamath/SCServo_Linux/blob/main/SyncWriteBuffer.h)*: Provides automatic buffer management for sync write operations, eliminating manual allocation/deallocation and preventing memory leaks.*
